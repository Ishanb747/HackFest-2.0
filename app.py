...
st.set_page_config(
    page_title="RuleForge — Policy Enforcement Engine",
    page_icon="⚖️",
    layout="wide",
...
<style>
  ...
  .kpi-label { font-size: .7rem; text-transform: uppercase; letter-spacing: 1.2px; color: #000000; font-weight: 600; margin-bottom: .4rem; }
...
st.markdown("""
<div class="hero-bar">
  <div class="hero-title">⚖️ RuleForge</div>
...
Watchdog Settings:
  Polling Interval: {WATCHDOG_INTERVAL} seconds""", language="text")
    ...
    with col_right:
        ...
        # Load live report
        from config import LIVE_REPORT_PATH
        ...
        if not LIVE_REPORT_PATH.exists():
            ...
            try:
                live_violations = json.loads(LIVE_REPORT_PATH.read_text(encoding="utf-8"))
                ...
                if isinstance(live_violations, list):
                    ...
                    # Last update time
                    last_update = datetime.fromtimestamp(LIVE_REPORT_PATH.stat().st_mtime)
